...
alongside `IsActive`. The structured conditions in `Builders/Conditions.fs` supply a reason; `when' value` supplies
none, and `when' value "reason"` supplies the one written at the call site.
