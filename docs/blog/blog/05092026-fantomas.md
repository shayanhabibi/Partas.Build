...
it absorbed the first and last. What it adds is that a stage declares the flags it reads, and the
command line, its help, its validation and `--explain` derive from that.

| Writing with                       | Flags | Help | Pipelines |
|------------------------------------|---|---|---|
...
The rest of this post ports the scripts above, starting with the file they all load.

```fsharp title="scripts/shared.fsx" showLineNumbers collapse={1-10, 12-25} {30-33}
...
```fsharp title="scripts/shared2.fsx" /Input\.option<[^>]+>/
...
module Options =
    ...
    let editorConfig = Input.option<string> "--editorconfig"
```
...
`parseArgs`{fsharp} turns the raw string into a `FormatConfig`{fsharp} itself. `InputSpec.map`{fsharp} does the same thing
once, where the option is declared, so nothing downstream ever sees the string:
...
one token left over can be the file. Declare the argument instead and the parser finds it:
