...
type ConditionsBuilder() =
    ...
    /// A literal condition, usually a value bound by an enclosing `inputs` CE.
    [<CustomOperation("when'")>]
    member inline _.when'([<InlineIfLambda>] build: BuildConditions, value: bool) = addCondition build (fun _ -> value)

    /// Runs the stage as a condition and takes its success as the answer.
    [<CustomOperation("when'")>]
    ...
    /// True when the current branch is any one of these.
...
type StageBuilder with
    /// Sets whether the stage is active. Conjoins onto whatever conditions the stage already has.
    [<CustomOperation("when'")>]
