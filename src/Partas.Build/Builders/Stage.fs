...
and [<EditorBrowsable(EditorBrowsableState.Advanced)>]
    ...
    [<CustomOperation>] member inline _.
        parallel'
        ([<InlineIfLambda>] build: BuildStage, ?flag: bool): BuildStage
        = build >> fun ctx -> { ctx with IsParallel = fun _ -> defaultArg flag true }
    [<CustomOperation>] member inline _.
        parallel'
        ([<InlineIfLambda>] build: BuildStage, [<InlineIfLambda>] condition): BuildStage
        = build >> fun ctx -> { ctx with IsParallel = condition }

    [<Obsolete("Use `parallel'` instead"); EditorBrowsable(EditorBrowsableState.Never)>]
    [<CustomOperation>] member inline this.paralle([<InlineIfLambda>] build, ?flag) = this.parallel'(build, ?flag = flag)
    [<Obsolete("Use `parallel'` instead"); EditorBrowsable(EditorBrowsableState.Never)>]
    [<CustomOperation>] member inline this.paralle([<InlineIfLambda>] build, [<InlineIfLambda>] condition) = this.parallel'(build, condition = condition)

    [<Obsolete("Use `parallel'` instead"); EditorBrowsable(EditorBrowsableState.Never)>]
    [<CustomOperation>] member inline this.concurrent([<InlineIfLambda>] build, ?flag) = this.parallel'(build, ?flag = flag)
    [<Obsolete("Use `parallel'` instead"); EditorBrowsable(EditorBrowsableState.Never)>]
    [<CustomOperation>] member inline this.concurrent([<InlineIfLambda>] build, [<InlineIfLambda>] condition) = this.parallel'(build, condition = condition)
    ...
    /// Adds a step that runs the command line the stage yields when it reaches this step.
    [<CustomOperation>] member this.
        run
    ...
    /// Adds a step that runs the <see cref="T:Partas.Build.Cmd"/> the stage yields when it reaches this step.
    [<CustomOperation>] member _.
        run
    ...
    /// <remarks>
    /// Overloads include: <code lang="fsharp">
    /// Async&lt;unit>
    /// StageContext -> unit
    /// StageContext -> int
    /// StageContext -> Async&lt;unit>
    /// StageContext -> Async&lt;int>
    /// StageContext -> Result&lt;unit, string>
    /// StageContext -> Task&lt;unit>
    /// StageContext -> Task&lt;int>
    /// StageContext -> Task
    /// StageContext -> Task&lt;Result&lt;unit, string>>
    /// </code>
    /// </remarks>
    [<CustomOperation>] member inline _.
        run
