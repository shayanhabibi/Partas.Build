...
module Output =
    ...
    module Logging =
        module httpHealthCheck =
            let inline start verbContainer url =
                Markup.escape url
                |> sprintf "Check %s ..."
                |> Markup.yellow
                |> vprintn verbContainer
            let inline failed message =
                Markup.escape message
                |> sprintf "Health check failed: %s"
                |> Markup.red
                |> printn
            let inline success verbContainer (url: string) =
                Markup.escape $"{url} is healthy!"
                |> Markup.green
                |> nprintn verbContainer
        module PipelineFailed =
            let print message =
                Markup.escape message
                |> Markup.red
                |> printn
        module Diagnostics =
            let makeTimingString (timeoutForStage: int) (timeoutForStep: int) retries =
                $"timeout: {timeoutForStage}ms. step timeout: {timeoutForStep}ms."
                + if retries > 0 then $" retries: {retries}." else ""
        module stageCondition =
            let private formatStarters stage timeoutForStage timeoutForStep retries msg =
                StageContext.getNamePath stage
                |> Markup.escape
                |> Markup.turquoise2
                |> Markup.bold
                |> sprintf "%s %s started." (Markup.escape msg)
                |> fun msg -> msg + " " + Diagnostics.makeTimingString timeoutForStage timeoutForStep retries
                |> Markup.grey
                |> Rule
                |> _.LeftJustified()
                |> vprint stage
            let private conditionStart stage timeoutForStage timeoutForStep =
                formatStarters stage timeoutForStage timeoutForStep stage.Retry "CONDITION STAGE"
            let private stageStart stage timeoutForStage timeoutForStep stageNum=
                formatStarters stage timeoutForStage timeoutForStep stage.Retry $"STAGE #{stageNum}"
            let private stepStart stage timeoutForStage timeoutForStep =
                $"%s{StageContext.buildCurrentStepPrefix stage |> Markup.escape}> sub-stage started %s{Diagnostics.makeTimingString timeoutForStage timeoutForStep stage.Retry}"
                |> Markup.grey
                |> vprintn stage
            let start stage index timeoutForStage timeoutForStep =
                match index with
                | StageIndex.Condition -> conditionStart stage timeoutForStage timeoutForStep
                | StageIndex.Stage i -> stageStart stage timeoutForStage timeoutForStep i
                | StageIndex.Step _ -> stepStart stage timeoutForStage timeoutForStep

            let private escapedNamePath stage = StageContext.getNamePath stage |> Markup.escape
            let inactive stage = function
                | StageIndex.Condition ->
                    $"CONDITION STAGE %s{escapedNamePath stage} is " + Markup.yellow "inactive"
                    |> Markup.grey
                    |> Rule
                    |> _.LeftJustified()
                    |> vprintn stage
                | StageIndex.Stage i ->
                    $"STAGE #{i} %s{escapedNamePath stage} is " + Markup.yellow "inactive"
                    |> Markup.grey
                    |> Rule
                    |> _.LeftJustified()
                    |> vprintn stage
                | StageIndex.Step _ ->
                    $"{StageContext.buildCurrentStepPrefix stage |> Markup.escape}> sub-stage is " + Markup.yellow "inactive"
                    |> Markup.grey
                    |> vprintn stage
            let finished stage (stageSw: System.Diagnostics.Stopwatch) isSuccess index =
                let escapedName = StageContext.getNamePath stage |> Markup.escape
                let color = if isSuccess then Markup.green else Markup.red
                match index with
                | StageIndex.Condition ->
                    let namePath =
                        escapedName
                        |> color
                        |> Markup.bold
                    $"CONDITION STAGE %s{namePath} finished. {stageSw.ElapsedMilliseconds}"
                    |> Markup.grey
                    |> Rule
                    |> _.LeftJustified()
                    |> nprintn stage
                | StageIndex.Stage i ->
                    ...
                    |> nprintn stage
                | StageIndex.Step _ ->
                    $"%s{StageContext.getNamePath stage |> Markup.escape}> sub-stage finished. {stageSw.ElapsedMilliseconds}ms."
                    |> Markup.grey
                    |> vprintn stage
...
module StageContext =
    ...
    let runHttpHealthCheckCancelableWithConfigRequest
        ...
        while shouldContinue && not cancellationToken.IsCancellationRequested do
            try
                Logging.httpHealthCheck.start ctx url
